...
def search_appid_by_name(term):
    ...
    try:
        ...
        # tiny_image is a small capsule art URL Steam's own search index
        # returns alongside each result -- reusing it (rather than guessing
        # a header.jpg CDN path from the appid) means we only ever show art
        # Steam has confirmed exists for that listing, which matters for
        # very new or barely-listed games where a guessed URL can 404.
        return [(item["id"], item["name"], item.get("tiny_image")) for item in items[:8]]
...
if query:
    if query.strip().isdigit():
        ...
        # No search-result art available for a raw appid (storesearch was
        # never called), so fall back to Steam's predictable header-image
        # CDN path. Best-effort only -- a brand-new or delisted appid can
        # 404 here, but st.image just renders a broken-image icon in that
        # case rather than crashing the app.
        art_url = f"https://cdn.akamai.steamstatic.com/steam/apps/{appid}/header.jpg"
    else:
        candidates = search_appid_by_name(query)
        if candidates:
            options = [f"{name} (appid {aid})" for aid, name, _ in candidates]
            choice = st.selectbox("Select the game:", options)
            idx = options.index(choice)
            appid, game_name, art_url = candidates[idx]
            # art_url here is Steam's own tiny_image from the search result --
            # confirmed to exist (Steam served it), which matters for very new
            # or barely-listed games where a guessed header.jpg path can 404.
            # It's a small capsule crop rather than a wide banner, but a real
            # small image beats a broken guess at a bigger one.
        else:
            st.warning(
...
if appid and st.button("Score this game", type="primary"):
    ...
    if "error" in result:
        ...
        # Best-effort only -- st.image renders a URL directly rather than
        # fetching/validating it first, so a 404 (possible for a very new
        # or delisted game) just shows a broken-image icon, not a crash.
        if art_url:
            st.image(art_url)
