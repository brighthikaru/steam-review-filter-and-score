...
**Summarization** — a third model, beyond the two required for the quality-filter/sentiment comparison above: a pretrained `facebook/bart-large-cnn` (TensorFlow backend, no separate PyTorch dependency) condenses the surfaced positive and negative reviews into a couple of plain-English sentences each. This replaced an earlier `t5-small` version — t5-small is a small, general-purpose model with no summarization-specific training, and on informal multi-review text it tended to just copy/paste fragments of the source reviews rather than genuinely condense them. bart-large-cnn is fine-tuned specifically for summarization (on the CNN/DailyMail dataset), at the cost of being roughly 6-7x larger (~400M vs. ~60M parameters) — a real memory tradeoff on a free-tier host, currently being validated the same way the t5-small version was: local proxy test, then a live deployment test scoring real games, before being trusted long-term. Summarized separately per side rather than blended — mixing "great gameplay" with "terrible bugs" in one prompt produced incoherent output with t5-small, and blending is unlikely to work better with a different model. Generation uses beam search (`num_beams=4`) with `no_repeat_ngram_size=3` to suppress repetition.
