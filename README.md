...
|---|---|---|
| `t5-small` (generic) | Safe on memory | Copy-pasted raw fragments rather than genuinely condensing them |
| `bart-large-cnn` (~400M params) | Genuinely coherent prose | OOM-crashed a live free-tier deployment on its first real request |
| `distilbart-cnn-12-6` | Never loaded | Only ships legacy PyTorch weights, no TF-native option, without adding PyTorch as a second DL framework |
